...
const GITIGNORE_LINES: &[(&str, &str)] = &[
    ...
    ("/.chops-search/model/", "~30 MB; the lockfile beside it is what to commit"),
];
...
pub fn init(root: &Path, with_page: bool) -> Result<()> {
    ...
    write_new(&root.join("chops-search.toml"), CONFIG, &mut created, &mut skipped)?;

    if with_page {
        let content = root.join("content");
        if content.is_dir() {
            write_new(&content.join("search.md"), SEARCH_PAGE, &mut created, &mut skipped)?;
        } else {
            skipped.push(format!(
